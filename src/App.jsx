...
// 100% Manually Verified Routine Data based on explicit user mapping
const routineData = {
  Saturday: {
    "slot1": { subject: "ME 2210 (Lab)", teacher: "MAK", room: "FS 201", type: "lab", duration: 3 },
    "slot4": { subject: "EEE 2281", teacher: "RAR", room: "FS 201", type: "theory" },
    "slot5": { subject: "ME 2203", teacher: "SMH", room: "FS 201", type: "theory" },
    "slot6": { subject: "ME 2207", teacher: "SMR", room: "FS 201", type: "theory" },
    "slot7": { subject: "ME 2208 (Lab)", teacher: "ES", room: "HE 203", type: "lab", duration: 3 },
  },
  Sunday: {
    "slot1": { subject: "MATH 2221", teacher: "MKH", room: "FS 201", type: "theory" },
    "slot2": { subject: "EEE 2281", teacher: "RAR", room: "FS 201", type: "theory" },
    "slot3": { subject: "ME 2203", teacher: "MWI", room: "FS 201", type: "theory" },
    // 4th, 5th, 6th periods are empty
    "slot7": { subject: "ME 2204 (Lab)", teacher: "MWI", room: "TBA", type: "lab", duration: 3 },
  },
  Monday: {
    // 1st, 2nd, 3rd periods are empty
    "slot4": { subject: "ME 2203", teacher: "MWI", room: "FS 201", type: "theory" },
    "slot5": { subject: "ME 2207", teacher: "HHH", room: "FS 201", type: "theory" },
    "slot6": { subject: "ME 2209", teacher: "MZS", room: "FS 201", type: "theory" },
    "slot7": { subject: "MATH 2222 (Lab)", teacher: "MMA", room: "HE 203", type: "lab", duration: 3 },
  },
  Tuesday: {
    // 1st, 2nd, 3rd periods are empty
    "slot4": { subject: "MATH 2221", teacher: "MBH", room: "FS 201", type: "theory" },
    "slot5": { subject: "ME 2209", teacher: "NI", room: "FS 201", type: "theory" },
    "slot6": { subject: "ME 2207", teacher: "SMR", room: "FS 201", type: "theory" },
    "slot7": { subject: "EEE 2282 (Machine Lab)", teacher: "RI", room: "Lab (West)", type: "lab", duration: 3 },
  },
  Wednesday: {
    "slot1": { subject: "ME 2209", teacher: "NI", room: "FS 201", type: "theory" },
    "slot2": { subject: "MATH 2221", teacher: "MKH", room: "FS 201", type: "theory" },
    "slot3": { subject: "EEE 2281", teacher: "RAR", room: "FS 201", type: "theory" },
    // 4th, 5th, 6th periods are empty
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
              <th className="p-4 font-semibold w-28 border-r border-slate-200 text-center sticky left-0 z-10 bg-slate-50 shadow-[1px_0_0_0_#e2e8f0]">Day</th>
              {timeSlots.map(slot => (
                <th key={slot.id} className={`p-3 font-semibold text-center whitespace-nowrap ${slot.isBreak ? 'w-16' : 'min-w-[120px]'}`}>
                  <div className="text-slate-900">{slot.isBreak ? (slot.id === 'break1' ? '☕' : '🍱') : slot.label}</div>
                  {!slot.isBreak && <div className="text-slate-500 font-normal mt-1 text-xs">{slot.time}</div>}
                </th>
              ))}
            </tr>
          </thead>
            ...
            {activeDays.map(day => {
              ...
              return (
                <tr key={day} className="border-b border-slate-100 last:border-none hover:bg-slate-50/50">
                  <td className="p-4 font-semibold text-slate-800 border-r border-slate-200 text-center bg-slate-50/90 sticky left-0 z-10 text-base shadow-[1px_0_0_0_#f1f5f9]">
                    {day}
                  </td>
                  ...
                  {timeSlots.map((slot, index) => {
                    if (slot.isBreak) {
                      return (
                        <td key={slot.id} className="p-2 bg-slate-100/50 border-l border-slate-100 text-center">
                          <div className="h-full flex flex-col items-center justify-center text-slate-400 py-4">
                            {slot.id === 'break1' ? <Coffee className="w-5 h-5" /> : <Utensils className="w-5 h-5" />}
                          </div>
                        </td>
                      );
                    }
                    ...
                    if (classInfo) {
                      ...
                      return (
                        <td 
                          ...
                          className="p-2 border-l border-slate-100 align-top h-full"
                        >
                          <div className={`p-3 rounded-xl border h-full flex flex-col gap-2 min-h-[90px] justify-center ${
                            classInfo.type === 'lab' 
                              ? 'bg-emerald-50 border-emerald-100 text-emerald-900' 
                              : 'bg-indigo-50 border-indigo-100 text-indigo-900'
                          }`}>
                            <div className="font-bold text-sm leading-snug">{classInfo.subject}</div>
                            <div className="flex flex-col gap-1.5 text-xs opacity-90 mt-1">
                      ...
                      return (
                        <td key={slot.id} className="p-2 border-l border-slate-100 align-top h-full">
                          <div className="h-full min-h-[90px] flex items-center justify-center text-slate-200 text-sm">
                            -
                          </div>
                        </td>
                      );
